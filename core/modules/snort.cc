...
char ** Snort::mSplit(char *str, char *sep, int max_strs, int *toks, char meta)
{
...
int Snort::ConvPort(char *port, char *proto)
{
...
void Snort::CreateRuleNode(int type)
{
...
int Snort::WhichProto(char *proto_str)
{
...
int Snort::ParseIP(char *addr, u_long *ip_addr, u_long *netmask)
{
...
int Snort::ParsePort(char *rule_port, u_short *hi_port, u_short *lo_port, char *proto, int *not_flag)
{
   char **toks;                /* token dbl buffer */
   int num_toks;               /* number of tokens found by mSplit() */
...
void Snort::ParseRuleOptions(char *rule)
{
   char **toks = NULL;
   int num_toks;
   char *idx;
...
void Snort::ParseRule(char *rule)
{
...
void Snort::ParsePattern(char *rule)
{
...
void Snort::ParseFlags(char *rule)
{
   char *fptr;
   char *fend;
...
void Snort::ParseMessage(char *msg)
{
   char *ptr;
   char *end;
   int size;
...
void Snort::ParseItype(char *number)
{
   char *type;
...
void Snort::ParseIcode(char *type)
{
   while(isspace(*type))
      type++;
...
CommandResponse Snort::Init(const bess::pb::SnortArg &arg){
  PassList = LogList = AlertList = nullptr;
  clear();
  return CommandAdd(arg);
}

CommandResponse Snort::CommandAdd(const bess::pb::SnortArg &arg){
  ParseRulesFile(arg.file().c_str());
  return CommandSuccess();
}

CommandResponse Snort::CommandClear(const bess::pb::EmptyArg &){
  clear();
  return CommandSuccess();
}
